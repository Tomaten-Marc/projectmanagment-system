...
export function Dashboard({ data }: { data: ProjectData }) {
  ...
  const nextActions = data.tasks.filter((item) => ["open","in_progress"].includes(item.status)).slice(0, 5);
  ...
  return <main className="page-content">
    ...
    <div className="dashboard-grid">
      ...
      <section className="panel"><header><div><Clock3 size={18} /><h2>Next Actions</h2></div></header><div className="panel-list numbered">{nextActions.map((item,index) => <div key={item.id}><span>{String(index+1).padStart(2,"0")}</span><div><strong>{item.title}</strong><small>{item.task_code} · {item.responsible ?? "Unassigned"}</small></div></div>)}{!nextActions.length && <p className="empty">No next actions.</p>}</div></section>
